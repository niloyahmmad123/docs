...
| `compaction_model`          | string  | ✗        | Model used for session compaction (summary generation). Can be a named model or an inline `provider/model` string. A `compaction_model` set on the agent's model takes precedence; when both are omitted, the agent's own model compacts. See the [Context & Compaction guide](../../guides/compaction/index.md). |
